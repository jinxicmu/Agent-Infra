...
状态：已部署到 Cloud Run 与运行目录 `Agent-Infra/`。开发分支 `fix/h3-sage41s` 保留在隔离目录 `Agent-Infra-h3-sage41s`。单首帧 I2V 与首尾帧 FL2V 均已通过真实端到端验证，见 [验证报告](H3_INPUTS_E2E_REPORT.md)。监控工作保持暂停。
...
| 输出画布 | 用首帧比例 + 0.98 MP 推导，曾实际输出 1248×832 | 保留源 ResolutionSelector：16:9、0.98 MP、32 对齐，得到 1344×768 |
...
时长使用外层有效值 5 秒，不使用子图内部未生效的 2 秒默认值；帧数表达式按源文件得到 124 帧，24 fps 时约 5.167 秒。请求 seed 仍使用云端分配并持久化的值，不能照搬编辑器 randomize 行为破坏任务恢复。

## API 兼容性

以源文件为准，此版本将 `ratio` 从 `adaptive` 改为 `16:9`；旧值返回 `INVALID_RATIO_FOR_WORKFLOW`，不会静默返回一个不同含义的画布。16:9 是源 selector 的档位名，32 像素取整后的精确尺寸是 1344×768。

公开 API 必须包含 first_frame，last_frame 可省略：单首帧归类为 i2v_first，首尾帧归类为 fl2v，使用同一源图与 checkpoint。查询 usage 返回实际输入图片数量。纯文本、仅末帧、重复帧角色仍被拒绝。调用方须使用 ratio: "16:9"；主设计与请求示例已同步更新。

源文件的“41s”是其注释记录的历史单次暖机成绩，演示只接首帧，不能直接等同于双帧 API 或两张卡并发的耗时。不承诺 41 秒；实际双 GPU API 测试计时单独记录在验证报告中。

## 验证与发布

- 完整 worker/cloudrun 自动化测试：45 passed（Firestore emulator，未访问生产数据库）。
- 新增验证：源图一致性、外层参数优先、提示词原样传递、直接首尾帧连接、任务间隔离、manifest 哈希覆盖、API ratio 明确拒绝旧值、可选末帧、输入计数与类型、重复/缺失首帧拒绝、发布准入开关。
...
发布必须协调 Cloud Run、worker 和调用方：先停止新增提交并等旧队列/活动租约处理完，再让两个空闲 worker 停止领取，发布一致的 manifest 与代码，更新调用方 ratio，启动 worker 后进行真实双帧冒烟验证，再恢复正常提交。不能把候选 workflow 文件单独覆盖到运行目录，也不能在旧任务执行途中切换 revision。失败时完整回滚云端和 worker 发布版本；涉及新 revision 的未完成任务必须先显式处理，禁止跨 revision 重放。

当前 workflow revision：`e3b6726a0b25c6585edaea0de973ac2bf6fdf926fc81e8c6d69869a071994606`。
